...
use App\Uploads\Donga300Attachment;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Request;

class Donga300 extends Boards
{
    // 테이블명, 필수
    protected string $table = "board_donga300s";

    // 프론트->백으로 넘어올 input name값. 반드시 디비의 컬럼명과 동일해야함, 필수
    // 여기에 정의 안되면 디비에 안들어감
    protected array $columns = [
        "user_id", "subject", "status_id", "homepage", "cnt_employed", "hit", "contents",
    ...
    {
        $this->attachmentTable = new Donga300Attachment();
            $this->insert($request);
            return $this->getId();
    }
