...
class KISA_SEED_CBC
{
    ...
    static function SEED_CBC_Decrypt(&$pbszUserKey, &$pbszIV, &$message, $message_offset, $message_length)
    {
        ...
        $pbszCipherText = array_pad(array(), count(array($message_length)), 0);
...
class Encryption
{
    public function decrypt($bszIV, $bszUser_key, $str)
    {
        ...
        for ($i = 0, $iMax = count($planBytes); $i < $iMax; $i++) {
    ...
    public function encrypt($bszIV, $bszUser_key, $str)
    {
        $planBytes = explode(",", $str);
        $keyBytes = explode(",", $bszUser_key);
        $IVBytes = explode(",", $bszIV);
...
function KISA_GET_DATA($data)
{
    ...
    $key = [
        '0x19,0x33,0x3F,0x8F,0x18,0x17,0x79,0xF1,0xE9,0xF3,0x94,0x37,0x0A,0xD4,0x05,0x89',
        '0x23,0x8D,0x66,0xA7,0x35,0xA2,0x11,0x81,0x6F,0xB3,0xD9,0x1A,0x36,0x16,0x25,0x03',
        false
    ];
