...
class CreateBoardReviewLatestsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('board_review_latests', function (Blueprint $table) {
            $table->id();
            $table->string("subject")->comment("제목");
            $table->longText("contents")->comment("내용");
            $table->string("account")->comment("학번 or 관리자 ID")->nullable();
            $table->smallInteger("user_type")->comment("1:학생 2: 관리자")->nullable();
    ...
    {
        Schema::dropIfExists('board_review_latests');
    }
}
