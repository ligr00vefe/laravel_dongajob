            ...
            <!------ 댓글 시작 ------>
                ...
                <div class="view-bottom">
                    <div class="btn-others">
                        @if ($prev_list)
                            <a href="/archive/reviewbefore/{{ $prev_list->id }}/view" class="prev-view switch-view-page">
                        ...
                        @if ($next_list)
                            <a href="/archive/reviewbefore/{{ $next_list->id }}/view" class="next-view switch-view-page">
                    ...
                    <div class="btn-wrap">
                        <div class="btn-right">
                            <form action="/archive/reviewbefore/{{ $list -> id }}/destroy" method="post" style="display: inline-block" onsubmit="if(!confirm('삭제하시겠습니까?')) { return false; }">
                                @csrf
                                @method("delete")
                            ...
                            @if($writer_check)
                                <a href="/archive/reviewbefore/{{ $list -> id }}/edit" class="btn-modify btn01">수정</a>
                            @endif

                            @if(session()->get('login_check'))
                                <a href="javascript:void(0)" class="btn-scrap btn01" onclick="__common.scrap({{ $list->id }}, '이전취업수기', '{{ $list->subject }}') ">스크랩</a>
                            @endif

                            <a href="/archive/reviewbefore" class="btn-list btn01">목록</a>

                            @if($login_check && !isStaffCheck(session()->get('donga_type')))
                                <a href="/archive/reviewbefore/create" class="btn-submit btn01 confirm-link">글쓰기</a>
                            @endif
                        </div>
                    </div>
