...
@section("title")
    동아대 관리자 - 최신 취업수기(최근 5년) {{ isset($list)? '수정' : '등록' }}
@endsection
...
@section("content")
    ...
    <section id="board_section" class="list_wrapper">
        <article id="list_head">
            ...
            <div class="head-info">
                <h1>최신 취업수기(최근5년) {{ isset($list)? '수정' : '등록' }}</h1>
            </div>
        </article><!-- article list_head end -->

        <div class="table02">
            <form action="/{{ ADMIN_URL }}/archive/reviewlatest/{{ isset($list) ? $list -> id.'/update' : 'store' }}" method="post" name="forms" data-route="/{{ ADMIN_URL }}/archive/reviewlatest" enctype="multipart/form-data">
                @csrf
                @if (isset($list))
                    @method("put")
                ...
                <table>
                    {{--<tr>--}}
                        {{--<th>이름</th>--}}
                        {{--<td>--}}
                            {{--<input type="text" name="name" class="_vali" data-tile="이름" value="{{ $list->name ?? '' }}">--}}
                        {{--</td>--}}
                    {{--</tr>--}}
                    {{--<tr>--}}
                        {{--<th>학과</th>--}}
                        {{--<td>--}}
                            {{--<input type="text" name="department" class="_vali" data-tile="학과" value="{{ $list->department ?? '' }}">--}}
                        {{--</td>--}}
                    {{--</tr>--}}
                    <tr>
                        <th>제목</th>
