...
@section("title")
    동아대 관리자 - 프로그램 참여후기 {{ isset($list)? '수정' : '등록' }}
@endsection
...
@section("content")
    ...
    <section id="board_section" class="list_wrapper">
        ...
        <div class="table02">
            <form action="/{{ ADMIN_URL }}/archive/reviewparticipate/{{ isset($list) ? $list -> id.'/update' : 'store' }}" method="post" name="forms" data-route="/{{ ADMIN_URL }}/archive/reviewparticipate" enctype="multipart/form-data">
                @csrf
                @if (isset($list))
                    @method("put")
