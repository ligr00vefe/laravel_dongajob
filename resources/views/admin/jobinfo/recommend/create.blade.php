...
@section("content")
    @php
        ...
        $recommend_alimi_list = get_recommend_alimi_lists();
    @endphp
    ...
    <section id="board_section" class="list_wrapper">
        <article id="list_head">
            ...
            <div class="head-info">
                <h1>추천채용 {{ isset($list)? '수정' : '등록' }}</h1>
            </div>
        </article> <!-- article list_head end -->

            <div class="table02">
                <form action="/{{ ADMIN_URL }}/jobinfo/recommend/{{ isset($list) ? '{id}/update' : 'create' }}" method="post" name="forms" data-route="/{{ ADMIN_URL }}/jobinfo/recommend" enctype="multipart/form-data">
                    @csrf
                    @if (isset($list))
                        @method("put")
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td class="radio_button_wrap">
                                @foreach($recruitment_list as $key => $value)
                                    <input type="radio" id="recommend_recruitment_{{ $key }}" class="_vail radio_txtbox" value="{{ $key }}" data-title="채용형태" name="category" {{ isset($list) && $list->category == $key ? 'checked' : '' }} {{--{{ !isset($list) ? 'checked' : '' }}--}}>
                        ...
                        <tr>
                            ...
                            <td class="radio_button_wrap">
                                @foreach($screening_method_list as $key => $value)
                                    <input type="radio" id="recommend_screening_{{ $key }}" class="_vail radio_txtbox" value="{{ $key }}" data-title="전형방법" name="screening_method" {{ isset($list) && $list->screening_method == $key ? 'checked' : '' }} {{--{{ !isset($list) ? 'checked' : '' }}--}}>
                        ...
                        <tr>
                            <th>직무 구분</th>
                            <td class="alimi_checkbox_wrap">
                                <p>* 체크항목에 따라 학생 알리미에 노출되며 미 체크시 학생 알리미에 노출되지 않습니다.</p>
                                <ul>
                                    @foreach($recommend_alimi_list as $key => $value)
                                        <li>
                                            <input type="checkbox" id="receipt_alimi_{{ $key }}" class="_vail tbl-checkbox" value="{{ $key }}"  name="alimi_category[]" {{ isset($category) && in_array($key, $category) ? 'checked' : '' }}>
                                            <label for="receipt_alimi_{{ $key }}" class="checkbox_txt_wrap"><span>{{ $value }}</span></label>
                                        </li>
                                     @endforeach
                                </ul>
                            </td>
                    ...
                    <div class="btn-wrap">
                        ...
                        @endif
                        <button type="button" onclick="history.back()" class="btn01 btn_cancel">취소</button>
